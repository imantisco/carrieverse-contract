...
// File: contracts/EmployeeTokenLock.sol
...
///@dev Employee token lockup contract
contract EmployeeTokenLock is ReentrancyGuard{
    ...
    modifier onlyAdmin() {
        require(_isAdmin(msg.sender), "EmployeeTokenLock: NOT_ADMIN");
        _;
    }

    modifier onlyLock(uint _lockupIndex){
        require(_isLockInfo(msg.sender), "EmployeeTokenLock: NOT_LOCKINFO");
        require(_isLockIndex(msg.sender, _lockupIndex), "EmployeeTokenLock: NOT_LOCKINDEX");
        _;
    }
    ...
    constructor(address[] memory _admins, address _token) {
        require(_admins.length == ADMINS_COUNT, "EmployeeTokenLock: ADMIN_COUNT");
        
        for( uint256 i; i < _admins.length; i++ ){
            require(address(0) != _admins[i], "EmployeeTokenLock: ZERO_ADDRESS");
        }
    ...
    function _removeLockup(address _account, address _receive, uint256 _lockupIndex) internal onlyAdmin(){
        require(_isLockInfo(_account), "EmployeeTokenLock: NOT_LOCKINFO");
        require(_isLockIndex(_account, _lockupIndex), "EmployeeTokenLock: NOT_LOCKINDEX");
